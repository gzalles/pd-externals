/* code for "foaAmbiPan~.c" pd class.  This takes 1 signal and 2 arguments which determine the elevation and azimuth. It also has 4 ouputs since we need to get W, X, Y and Z.
 ...
 Notes: we have to decide which ordering we will use. WXYZ or FuMa in this case. We have to be careful about radians v. degrees!
 */
...
typedef struct _foaAmbiPan_tilde {
    ...
    t_inlet *x_in2;//inlet 2 (for messages - azi)
    t_inlet *x_in3;//inlet 3 (for messages - elev.)
...
void foaAmbiPan_tilde_dsp(t_foaAmbiPan_tilde *x, t_signal **sp)
{
    dsp_add(foaAmbiPan_tilde_perform, //following arguments passed to perform
            7,            //number of pointers (seven element to follow)
            x,            //object itself [1]
            sp[0]->s_vec, //sp[0] is the first in-signal [2]
            sp[1]->s_vec, //sp[1] points to the out-signal 1 (W) [3]
            sp[2]->s_vec, //sp[2] points to the out-signal 2 (X) [4]
            sp[3]->s_vec, //sp[3] points to the out-signal 3 (Y) [5]
            sp[4]->s_vec, //sp[4] points to the out-signal 4 (Z) [6]
            sp[0]->s_n);  //length of first in-signal (block size = N) [7]
}
...
void foaAmbiPan_tilde_setup(void) {
    ...
    //this one has one signal inlet and 2 extra float inlets.
