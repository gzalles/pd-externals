...
typedef struct _foaRot_tilde {
    ...
    //      *x_in1 = MAINSIGNALIN (I believe this one is automatic )
    t_inlet *x_in2; // X
    t_inlet *x_in3; // Y
    t_inlet *x_in4; // Z
...
//    roll (Xrot) : Y’ = Y * cos(b) – Z * sin(b) & Z’ = Y * sin(b) + Z * cos(b)
    ...
    while (n--) {
        ...
        //W always stays the same (its an omni signal, rotating does nothing)
        //X Out
...
//signal processing method definition
...
void foaRot_tilde_dsp(t_foaRot_tilde *x, t_signal **sp)
{
    dsp_add(foaRot_tilde_perform, //following arguments passed to perform
            10,            //number of pointers [ten total, 8 for sigs, 1 for block_size, 1 for x]
            x,            //object itself
